...
## Спринт 103: Единая таблица Claude/Codex
**[Role: Desktop App Engineer]**

- [x] {{TASK:103.1}} Убрать подписи разделов и выровнять строки
  **ПОДРОБНОСТИ:**
  - **Что сделано:** В Slint UI убраны подписи "КЛОД" и "КОДЕКС", оставлен один общий заголовок колонок для Claude-аккаунтов и Codex-строки.
  - **Критерии приемки:** Все строки читаются как одна таблица с колонками "Логин", "Тариф", "5ч сессия", "7д лимит", "7д Design"; правые галочки имеют стабильный внутренний отступ от края окна.

- [x] {{TASK:103.2}} Добавить provider-иконку в строку
  **ПОДРОБНОСТИ:**
  - **Что сделано:** В `AccountView` добавлен `provider_kind`; в Slint UI подключены локальные SVG `claude-code.svg` и `openai-chatgpt.svg`, иконка показывается между левой отметкой строки и логином.
  - **Критерии приемки:** У Claude-аккаунтов видна иконка Claude Code, у Codex/ChatGPT-строки видна иконка ChatGPT; текстовые заголовки секций не нужны.

- [x] {{TASK:103.3}} Обновить источник колонки 7д Design
  **ПОДРОБНОСТИ:**
  - **Что сделано:** Для Claude Design добавлен fallback со старого `seven_day_omelette` на текущий отдельный API-бакет `seven_day_sonnet`; добавлен регрессионный тест.
  - **Критерии приемки:** Если текущий Claude API возвращает `seven_day_omelette: null`, колонка 7д Design всё равно заполняется отдельным недельным лимитом из живого ответа API.

## Спринт 102: Выбор источника tray-иконок
**[Role: Desktop App Engineer]**

- [x] {{TASK:102.1}} Добавить правую галочку выбора источника трея
  **ПОДРОБНОСТИ:**
  - **Что сделано:** В строки Claude и Codex добавлена правая галочка, независимая от левой галочки активного Claude-аккаунта.
  - **Критерии приемки:** В UI видно, какая строка управляет двумя tray-иконками; клик по правой галочке меняет источник без переключения активного Claude-аккаунта.

- [x] {{TASK:102.2}} Сохранять выбранный источник трея
  **ПОДРОБНОСТИ:**
  - **Что сделано:** В `accounts_meta.json` добавлено поле `tray_source` со значением `claude:<account_id>` или `codex`.
  - **Критерии приемки:** Выбор сохраняется после перезапуска; при удалении выбранного Claude-аккаунта трэй возвращается к активному Claude-аккаунту.

- [x] {{TASK:102.3}} Обновлять tray-иконки из выбранной строки
  **ПОДРОБНОСТИ:**
  - **Что сделано:** Фоновые обновления Claude и Codex больше не перетирают выбор друг друга; обе tray-иконки берут 5ч/7д проценты из строки с правой галочкой.
  - **Критерии приемки:** Если выбрана строка Codex, в трее отображаются проценты Codex; если выбран Claude-аккаунт, отображаются его проценты.

## Спринт 101: Раздел Codex и лимиты 5ч/7д
**[Role: Desktop App Engineer]**

- [x] {{TASK:101.1}} Добавить источник данных Codex
  **ПОДРОБНОСТИ:**
  - **Что сделано:** Добавлен Rust-модуль `codex_usage`, который сначала читает официальный локальный `codex app-server` (`account/rateLimits/read`) и преобразует `usedPercent` в существующий формат метрик. Журнал Codex используется как fallback.
  - **Критерии приемки:** 5-часовой лимит берётся из `primary.usedPercent`, недельный лимит из `secondary.usedPercent`, reset-время переводится в формат, который уже понимает UI-таймер.

- [x] {{TASK:101.2}} Добавить данные логина, тарифа и сессии Codex
  **ПОДРОБНОСТИ:**
  - **Что сделано:** Логин и тариф добираются из локального `~/.codex/auth.json`, идентификатор сессии берётся рядом с последним событием лимитов в журнале Codex.
  - **Критерии приемки:** В Codex-разделе видны логин, тариф, процент 5ч и 7д лимитов, время до сброса.

- [x] {{TASK:101.3}} Разделить интерфейс на "КЛОД" и "КОДЕКС"
  **ПОДРОБНОСТИ:**
  - **Что сделано:** В Slint UI верхний блок переименован в "КЛОД"; ниже добавлен отдельный блок "КОДЕКС" с колонками "Логин", "Тариф", "5ч лимит", "7д лимит".
  - **Критерии приемки:** Claude-аккаунты и Codex-сессия визуально разделены, существующие действия Claude не ломаются.

- [x] {{TASK:101.4}} Обновлять Codex автоматически
  **ПОДРОБНОСТИ:**
  - **Что сделано:** Добавлен отдельный фоновый обновитель Codex раз в минуту; обратный отсчёт до сброса пересчитывается каждую секунду вместе с Claude.
  - **Критерии приемки:** Codex-строка обновляется без ручного запуска `codex status`.

- [x] {{TASK:101.5}} Покрыть парсер тестами
  **ПОДРОБНОСТИ:**
  - **Что сделано:** Добавлены тесты на разбор `codex.rate_limits`, выбор самого свежего события из журнала и извлечение логина/тарифа из локального Codex auth.
  - **Критерии приемки:** `cargo test` проходит.
